...
contract('DappToken',(accounts)=>{
    ...
    it('approves tokens for delegated transfer',()=>{
        return DappToken.deployed().then((instance)=>{
            ...
            assert.equal(receipt.logs[0].args._value,100,'logs the value to be authorized')
    ...
    it('handles delegated token transfers',()=>{
        return DappToken.deployed().then((instance)=>{
            ...
            assert.equal(receipt.logs.length,1,'triggers one event');
            assert.equal(receipt.logs[0].event,'Transfer','should be a approval event');
            ...
            assert.equal(receipt.logs[0].args._value,10,'logs the value to be transferred');
            return tokenInstance.balanceOf(fromAccount);
        }).then((balance)=>{
            assert.equal(balance,90,'deducts the amount from the account');
            return tokenInstance.balanceOf(toAccount);
        }).then((balance)=>{
            assert.equal(balance,10,'adds the amount to the account');
